  ...
  constructor(scope: cdk.App, id: string, props: GithubActionRoleStackProps) {
        ...
        provider.openIdConnectProviderArn,
        {
          StringLike: {
            "token.actions.githubusercontent.com:sub": [
              "repo:A1MSolutions/policyconnector:*",
              "repo:A1MSolutions/humanservices:*",
            ],
          },
        },
      ),
    ...
    const devRole = new cdk.aws_iam.Role(this, "developer-role", {
      ...
      assumedBy: new cdk.aws_iam.ArnPrincipal(
        `arn:aws:iam::${this.account}:user/${props.developerUsername}`,
      ),
    });

    // Create deployment policy that will be shared by both roles
    const deploymentPolicy = new cdk.aws_iam.ManagedPolicy(
      this,
      "DeploymentPolicy",
      {
        statements: [
          ...
          new cdk.aws_iam.PolicyStatement({
            ...
            actions: ["lambda:*"],
            resources: [
              `arn:aws:lambda:${this.region}:${this.account}:function:a1m-eregs-hs-*`,
            ],
          }),
          // S3 permissions
          new cdk.aws_iam.PolicyStatement({
          ...
          new cdk.aws_iam.PolicyStatement({
            ...
            resources: [
              `arn:aws:secretsmanager:${this.region}:${this.account}:secret:a1m-eregs-hs-*`,
            ],
          }),
          // RDS permissions
          new cdk.aws_iam.PolicyStatement({
            ...
            actions: ["rds:*"],
            resources: [
              `arn:aws:rds:${this.region}:${this.account}:db:a1m-eregs-hs-*`,
              `arn:aws:rds:${this.region}:${this.account}:cluster:a1m-eregs-hs-*`,
            ],
          }),
          ...
          new cdk.aws_iam.PolicyStatement({
            ...
            resources: [
              `arn:aws:logs:${this.region}:${this.account}:log-group:/aws/lambda/a1m-eregs-hs-*`,
            ],
          }),
          ...
          new cdk.aws_iam.PolicyStatement({
            ...
            resources: [
              `arn:aws:iam::${this.account}:role/delegatedadmin/developer/*`,
            ],
          }),
